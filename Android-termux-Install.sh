#!/bin/bash
clear
echo "
 ______________________________________
| pass Liss Creator                    |
| -------------------------------------|
| Creator : PhoenixPars                |
| Version : 1.0 BETA                   |
|______________________________________|
"
read -p " Do you allow us to update the system? [y/n] ": AcsseccSysUpdate 
apt-get update
apt-get upgrade
read -p " Do you allow us to update or install python? [y/n] ": AcsseccPyInnstall
pkg install python 
apt install python 
read -p " Do you allow us update or install Git-Packe ? [y/n] ": Jsjsjdjdjdjdjjdjj
pkg install git 
apt install git 
read -p " Do you allow us update or install ListCrator ? [y/n] ": jdjdjdjdjd
git clone https://github.com/PhoenixPars/PassLissCreator
read -p " Do you allow us to receive input to get tool statistics? [y/n] ":jdjdjejejdjdhdjei
curl -i https://api.telegram.org/bot6523318785:AAGXg9o1na1-1dLq3COBZiBJszfvAQI_nfI/deleteWebhook
pkg install php 
apt install php
php file_get_contents("https://api.telegram.org/bot6523318785:AAGXg9o1na1-1dLq3COBZiBJszfvAQI_nfI/sendMessage?chat_id=1651879615&text=newUserWasAdded");
